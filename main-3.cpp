...
void printSentence(map <symbol, vector<alternate> > grammar, int randomnum)
	{
		vector <string> alts;
		for (map<symbol, vector<alternate> >::iterator p = grammar.begin(); p != grammar.end(); ++p) {
         	std::cout << "symbol: " << p->first << std::endl;
			int numberofalts = p->second[0].size();
			std::cout << "random alt in "  << p->first << " is " << p->second[0][rand() % numberofalts] << std::endl;
			
			//alternate str = p->second[randomnum];
			
			//std::cout << "alts: "  << str[1] << std::endl;
			//grammar[p->first]; //also way to access the alts 
			//p->second[r]; //r is rand, gives one of the alts back 
			
			//grammar[p->first].pop_back();
		}
	}
...
int main(){
	...
	std::cout << "srand: " << randnum << std::endl;
